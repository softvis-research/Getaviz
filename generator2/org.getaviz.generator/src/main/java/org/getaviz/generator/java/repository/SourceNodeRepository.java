...
public class SourceNodeRepository {
    ...
    public void loadNodesByType(JavaNodeTypes type) {
        ...
        connector.executeRead("MATCH (n:" + type + ") RETURN n")
                .forEachRemaining((result) -> {
                    ...
                    addNodeByProperty(sourceNode);

                    counter.addAndGet(1);
                });

        log.info(counter.get() + " Nodes added with type " + type);
    }

//    public void loadNodesByPropertyValue(JavaNodeProperties property, String value) {
//
//        AtomicInteger counter = new AtomicInteger(0);
//
...
//    public void loadNodesByRelation(JavaRelationLabels relationType){
//        loadNodesByRelation(relationType, false);
//    }

    public void loadNodesByRelation(JavaRelationLabels relationType, JavaNodeTypes nodeType, boolean recursive){
        loadNodesByRelation(relationType, nodeType, recursive, true);
    }

    public void loadNodesByRelation(JavaRelationLabels relationType, JavaNodeTypes nodeType, boolean recursive, boolean forward){
        Set<Long> nodeIds = nodeById.keySet();
        loadNodesByRelation(nodeIds, relationType, nodeType, recursive, forward);
    }

    public void loadNodesByRelation(Set<Long> nodeIds, JavaRelationLabels relationType,JavaNodeTypes nodeType, boolean recursive, boolean forward){
        String nodeIDString = computeNodeIDString(nodeIds);
        String relatedNodesStatement = "";

        if (forward) {
            relatedNodesStatement = "MATCH (m:" + nodeType + ")-[:" + relationType.name() + "]->(n) WHERE ID(m) IN " + nodeIDString + " RETURN m, n";
        } else {
        ...
        if (recursive && !newNodeIds.isEmpty()){
            loadNodesByRelation(newNodeIds, relationType, nodeType, true, forward);
        }
    }
...
//    }
    ...
    public Collection<Node> getRelatedNodes(Node node, JavaRelationLabels relationLabel, Boolean direction){
...
//    public Collection<Node> getNodesByLabel(JavaNodeLabels label){
...
//    }
    ...
    public Collection<Node> getNodesByProperty(String nodeType){
//        Collection<Node> nodesByID = getNodes();
        List<Node> nodesByProperty = new ArrayList<>();
        connector.executeRead("MATCH (n:" + nodeType + ") RETURN n")
                 .forEachRemaining((result) -> nodesByProperty.add(result.get("n").asNode()));
...
//
//
//    public Collection<Node> getNodesByLabelAndProperty(JavaNodeLabels label, String property, String value){
