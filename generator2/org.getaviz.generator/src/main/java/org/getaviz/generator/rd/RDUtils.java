...
public class RDUtils {
	...
	public static Iterator<Node> getMethods(Long disk) {
		return connector.executeRead(
			"MATCH (n)-[:CONTAINS]->(d:DiskSegment)-[:VISUALIZES]->(method) " +
			"WHERE ID(n) = " + disk + " AND (method:Method OR method:Function) " +
			"RETURN d " +
	...
	public static Iterator<Node> getData(Long disk) {
		return connector.executeRead(
			"MATCH (n)-[:CONTAINS]->(d:DiskSegment)-[:VISUALIZES]->(field) " +
			"WHERE ID(n) = " + disk + " AND (field:Field OR field:Variable OR field:EnumConstant) " +
			"RETURN d " +
			"ORDER BY field.hash"
		).stream().map(s -> s.get("d").asNode()).collect(Collectors.toList()).listIterator();
