...
public class SettingsConfiguration {
	...
	public ShowVersions getShowVersions() {
		...
		case "latest":
			return ShowVersions.LATEST;
		default:
			return ShowVersions.ALL;
		}
	}
	...
	public FamixParser getParser() {
		switch (config.getString("structure.parser", "verveinej")) {
		case "jdt2famix":
	...
	public BuildingType getBuildingType() {
		switch (config.getString("city.building_type", "original")) {
		case "panels":
	...
	public Schemes getScheme() {
		switch (config.getString("city.scheme", "types")) {
		case "visibility":
	...
	public ClassElementsModes getClassElementsMode() {
		String property = config.getString("city.class_elements_mode", "methods_and_attributes");
		switch (property) {
		case "methods_only":
	...
	public ClassElementsSortModesCoarse getClassElementsSortModeCoarse() {
		String property = config.getString("city.class_elements_sort_mode_coarse", "methods_first");
		switch (property) {
		case "unsorted":
	...
	public ClassElementsSortModesFine getClassElementsSortModeFine() {
		String property = config.getString("city.elements_sort_mode_fine", "scheme");
		switch (property) {
		case "unsorted":
	...
	public Layout getBrickLayout() {
		String property = config.getString("city.brick.layout", "progressive");
		switch (property) {
		case "straight":
	...
	public SeparatorModes getPanelSeparatorMode() {
		String property = config.getString("city.panel_separator_mode", "separator");
		switch (property) {
		case "none":
	...
	public BuildingMetric getOriginalBuildingMetric() {
		String property = config.getString("city.original_building_metric", "none");
		switch (property) {
		case "nos":
	...
	public RDClassSize getRDClassSize() {
		...
		case "number_of_statements":
			return RDClassSize.NUMBER_OF_STATEMENTS;
		case "betweenness_centrality":
			return RDClassSize.BETWEENNESS_CENTRALITY;
		default:
			return RDClassSize.NONE;
		}
	}
	...
	public ClassHeight getRDClassHeight() {
		...
		case "number_of_incidents":
			return ClassHeight.NUMBER_OF_INCIDENTS;
		default:
			return ClassHeight.STATIC;
		}
	}
	...
	public ClassColorMetric getRDClassColorMetric() {
		...
		case "stk":
			return ClassColorMetric.STK;
		case "change_frequency":
			return ClassColorMetric.CHANGE_FREQUENCY;
		default:
			return ClassColorMetric.STATIC;
		}
	}
	...
	public MetricRepresentation getMetricRepresentation() {
		String property = config.getString("rd.metric_representation", "none");
		switch (property) {
		case "height":
	...
	public InvocationRepresentation getInvocationRepresentation() {
		String property = config.getString("rd.invocation_representation", "none");
		switch (property) {
		case "moving_spheres":
	...
	public Variant getVariant() {
		String property = config.getString("rd.variant", "static");
		switch (property) {
		case "dynamic":
	...
	public String getPackageEvenTexture() {
		return config.getString("plant.package.even_texture",
				"<ImageTexture url='pics/freeGrass.png' scale='false' />");
	}
	...
	/**
	 * Depending on this Variable the Dynamix Visualization will be created, it can
	 * either be in a static or dynamic way
	 */
	...
	/**
	 * Sets in which way the Historic Evolution of the analyzed Software should be
	 * represented, it can either be in a static or dynamic way
	 */
	...
	public static enum Schemes {
		/**
		 * The class elements are sorted and colored corresponding to there visibility
		 * modifiers.
		 * 
		...
		/**
		 * The class elements are sorted and colored associated to type/functionality of
		 * the method.
		 * 
	...
	public static enum Methods {
		...
		public static enum SortPriorities_Types {
			...
			/**
			 * Every other type that isn't specified by the other constants in this field.
			 */
	...
	public static enum Bricks {
		...
		public static enum Layout {
			...
			/**
			 * One-dimensional bricks layout, where the segments simply are placed on top of
			 * the other.
			 */
			STRAIGHT,

			/**
			 * Three-dimensional brick layout, where the base area is computed depending on
			 * the {@link CitySettings#SET_CLASS_ELEMENTS_MODE SET_CLASS_ELEMENTS_MODE}.<br>
			 * If only methods are shown, the base area is computed by the number of
			 * attributes and vice versa.<br>
			 * In case of methods and attributes are shown, the base area is computed by the
			 * sum of the numbers of attributes and methods inside the class.
			 * <p>
			 * When {@link CitySettings#SET_CLASS_ELEMENTS_MODE SET_CLASS_ELEMENTS_MODE} is
			 * set to {@code METHODS_AND_ATTRIBUTES}, the {@code BALANCED} layout and
			 ...
			 * Three-dimensional brick layout, where the base area is computed depending on
			 * the {@link CitySettings#SET_CLASS_ELEMENTS_MODE SET_CLASS_ELEMENTS_MODE}.<br>
			 * If only methods are shown, the base area is computed by the number of methods
			 * and vice versa. So the aspect lies on only one type of element of a class and
			 * is visualized.
			 * <p>
			 * When {@link CitySettings#SET_CLASS_ELEMENTS_MODE SET_CLASS_ELEMENTS_MODE} is
			 * set to {@code METHODS_AND_ATTRIBUTES}, the {@link Layout#BALANCED
	...
	public enum Panels {
		 ...
		 * The panels can either touch each other without a gap, leave a gap between
		 * them, or fill the space with a separator of a defined color.
		 * 
		...
		public static enum SeparatorModes {
			...
			/**
			 * No space between the panels and they are placed on top of each other.
			 */
			NONE,

			/**
			 * The panels have a free space between them and don't touch each other.
			 * 
			...
			/**
			 * Between the panels separators are placed with a fix height and color.
			 * 
