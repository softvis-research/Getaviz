...
class Famix2Hismo extends WorkflowComponentWithModelSlot {
	val static hismoFactory = new HismoFactoryImpl()
	...
	def private HISMOClassHistory toHismoHistory(FAMIXStructure st) {
		...
		if (famixelement instanceof FAMIXNamespace) {
			actualNamespaceHistory = hismoDocument.elements.filter(HISMONamespaceHistory).findFirst[value === famixelement.fqn]
