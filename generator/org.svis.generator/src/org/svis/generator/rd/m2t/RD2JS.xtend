...
class RD2JS {
	...
	def private toEvents(List<DiskSegment> segments) {
		...
		for (segment: segments) {
			if (segment.frequency != 0) {
				actions.add('''{"time" : 0, "action" : "changeColor", "parameters":{"node_id": "«segment.diskSegmentMaterialId»", "type": "show_and_return", "duration": "«segment.frequency»", "target_value": "1.0 1.0 1.0" }}''')
			}
			if (segment.luminance != 0) {
				actions.add('''{"time" : 0, "action" : "changeColor", "parameters":{"node_id": "«segment.diskSegmentMaterialId»", "type": "show_and_return", "duration":1000, "target_value": "«((1/segment.luminance))» «((1/segment.luminance))» «((1/segment.luminance))»" }}''')
			}
		}
	...
	def diskSegmentInvocationsJs(Resource resource,List<DiskSegment> diskSegments) {
		...
		if(diskSegmentInvocations.size == 0 || RDSettings::VARIANT == Variant::STATIC) {
	...
	def private toEventsInvocations(List<DiskSegmentInvocation> invocations) {
		...
		for (invocation: invocations.sortBy[start]) {
			...
			if (RDSettings::INVOCATION_REPRESENTATION == InvocationRepresentation::MOVING_SPHERES
				|| RDSettings::INVOCATION_REPRESENTATION == InvocationRepresentation::MOVING_FLASHING) {		
				if (caller !== null) {
					actions += '''{"time" : «invocation.start», "action" : "addNode", "parameters":{"node_id": "invocation_«invocation.name»", "size": "2", "node_type":"Sphere", "initial_position": "«caller.diskSegmentCenter»", "use_label" : true, "label": "«invocation.fqn.replaceAll("^.*\\.","")»" }}'''
					actions += '''{"time" : «invocation.start», "action" : "moveNodeCircular", "parameters":{"node_id": "invocation_«invocation.name»", "duration": "«timePerAnimation»", "target_value": "«invocation.diskSegmentCenter»" }}'''
					actions += '''{"time" : «invocation.stop - timePerAnimation», "action" : "moveNodeCircular", "parameters":{"node_id": "invocation_«invocation.name»", "duration": "«timePerAnimation»", "target_value": "«caller.diskSegmentCenter»" }}'''
				} else {
					actions += '''{"time" : «invocation.start», "action" : "addNode", "parameters":{"node_id": "invocation_«invocation.name»", "size": "2", "node_type":"Sphere", "initial_position": "«invocation.diskSegmentCenter»", "use_label" : true, "label": "«invocation.fqn.replaceAll("^.*\\.","")»" }}'''	
				}
				actions += '''{"time" : «invocation.stop», "action" : "removeNode", "parameters":{"node_id": "invocation_«invocation.name»"}}'''
			}	
			
			if (RDSettings::INVOCATION_REPRESENTATION == InvocationRepresentation::FLASHING_METHODS
				|| RDSettings::INVOCATION_REPRESENTATION == InvocationRepresentation::MOVING_FLASHING) {
				actions += '''{"time" : «invocation.start», "action" : "changeColor", "parameters":{"node_id": "«invocation.diskSegmentMaterialId»", "count": 1, "duration": "«timePerAnimation»", "target_value": "1.0 1.0 1.0" }}'''
				actions += '''{"time" : «invocation.stop - timePerAnimation», "action" : "changeColor", "parameters":{"node_id": "«invocation.diskSegmentMaterialId»", "count": 1, "duration": "«timePerAnimation»", "target_value": "«invocation.diskSegmentColor»" }}'''
			}
		}		
	...
	def private diskSegmentCenter(DiskSegmentInvocation segmentInvocation) {
		...
		val pointX = (Math::cos(targetAngle) * segment.outerRadius - (Math::cos(targetAngle) * segment.innerRadius)) / 2.0 + (Math::cos(targetAngle) * segment.innerRadius)
		val pointY = (Math::sin(targetAngle) * segment.outerRadius - (Math::sin(targetAngle) * segment.innerRadius)) / 2.0 + (Math::sin(targetAngle) * segment.innerRadius)
		val pointZ = 2.0
		...
		sortedVersions.forEach[level, versions|
			...
			for (version : versions) {
				...
				if (diskTargetValues.get(disk) != targetValue) {
					actions += ('''{"time" : «(((version.level * versionSpeed))) + startTime», "action" : "changeScale", "parameters":{"count": 1, "node_id": "«nodeId»", "duration":"«diskVersionAnimationSpeed»", "target_value":"«targetValue»" }}''')
				}
			...
			for (disk : disks) {
				if (!availableDisks.contains(disk)) {
					...
					if (diskTargetValues.get(disk) != targetValue) {
						actions += ('''{"time" : «(((level * versionSpeed))) + startTime», "action" : "changeScale", "parameters":{"count": 1, "node_id": "«nodeId»", "duration":"«diskVersionAnimationSpeed»", "target_value":"«targetValue»" }}''')
					}
					diskTargetValues.put(disk, targetValue)
				}
			}
			...
			for (version : segmentVersions) {
				...
				if (version.scale > 0.001) {
			 	...
			 	if (segmentTargetValues.get(diskSegment) != targetValue) {
						actions += '''{"time" : «version.level * versionSpeed + startTime», "action" : "changeOuterRadius", "parameters":{"count": 1, "node_id": "«nodeId»", "duration":"«segmentVersionAnimationSpeed»", "target_value":"«targetValue»" }}'''
				}	
			...
			for (segment: diskSegments) {
				if (!availableSegments.contains(segment)) {
					...
					if (segmentTargetValues.get(segment) != targetValue) {
						actions += '''{"time" : «level * versionSpeed + startTime», "action" : "changeOuterRadius", "parameters":{"count": 1, "node_id": "«nodeId»", "duration":"«segmentVersionAnimationSpeed»", "target_value":"«targetValue»" }}'''
					}
	...
	def toMultipleDiskVersionEvents(List<Disk> multipleDisks,List<DiskSegment> multipleDiskSegments,List<DiskVersion> diskVersions){
		if(!(RDSettings::EVOLUTION_REPRESENTATION == EvolutionRepresentation::MULTIPLE_DYNAMIC_EVOLUTION)) {
	 		return ""
	 	}
		...
		val tmpactions = toDiskEvents(timestampsDisk,allTimestamps)+ toDiskSegmentEvents(timestampsDiskSegment,allTimestamps,multipleDiskSegments)
	...
	def toDiskEvents(HashMap<String, ArrayList<String>> timestamps, List<String> allTimestamps) {
		...
		for (String timestamp: allTimestamps) {
			for (String tmpNodeId: timestamps.get(timestamp)) {
				...
				actions += '''{"time" : «time», "action" : "changeScale", "parameters":{"count": 1, "node_id": "«nodeId»_SCALE", "duration":"«diskVersionAnimationSpeed»", "target_value":"1.0 1.0 1.0" }}'''
			}
			time += segmentVersionAnimationSpeed + 200 
			for (String tmpNodeId: timestamps.get(timestamp)) {
				val nodeId = tmpNodeId
				actions += '''{"time" : «time», "action" : "changeScale", "parameters":{"count": 1, "node_id": "«nodeId»_SCALE", "duration":"«diskVersionAnimationSpeed»", "target_value":"0.00001 0.00001 0.00001" }}'''
			}
			time += versionSpeed
		}
		return actions
	...
	def toDiskSegmentEvents(HashMap<String, ArrayList<String>> timestamps, List<String> allTimestamps,List<DiskSegment> diskSegments) {
		var actions = new ArrayList<String>()
		var time = startTime
		for (String timestamp: allTimestamps) {
			for (String tmpNodeId: timestamps.get(timestamp)) {
				...
				val nodeId = tmpNodeId	
				actions += '''{"time" : «time», "action" : "changeOuterRadius", "parameters":{"count": 1, "node_id": "«nodeId»__RECTANGULARTORUS", "duration":"«segmentVersionAnimationSpeed»", "target_value":"«diskSegment.outerRadius»" }}'''
			}
			time += segmentVersionAnimationSpeed + 200 
			for (String tmpNodeId: timestamps.get(timestamp)) {
				val nodeId = tmpNodeId
				actions += '''{"time" : «time», "action" : "changeOuterRadius", "parameters":{"count": 1, "node_id": "«nodeId»__RECTANGULARTORUS", "duration":"«segmentVersionAnimationSpeed»", "target_value":"0.0" }}'''
			}
			time += versionSpeed
		}
		return actions	
