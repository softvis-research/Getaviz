...
class RD2X3DCompressed {
	...
	def toX3DBody(Resource resource) { 
		...
		if(RDSettings::METHOD_DISKS == true) {
			disks = disks.filter[type !== "FAMIX.Method"].toList
		}
		if(RDSettings::DATA_DISKS == true) {
		...
		val structures = disks.filter[type == "FAMIX.Class"||type == "FAMIX.ParameterizableClass"||type == "FAMIX.Enum" || type == "FAMIX.AnnotationType"].toList	
	...
	def String toRD(List<Disk> disks,List<DiskSegment> diskSegments,TreeMap<String,List<DiskSegment>> dsGrouped, TreeMap <String,List<Disk>> dGrouped) '''
			««« -->Disks
	...
	«FOR segs : segments.values»
		«FOR segment :segs»
		«IF(segment.color == RDSettings::DATA_COLOR)»
			...
			<Transform DEF='«famix.createID(invocation.fqn)»' 
				...
				rotation='0 0 1 1.57' scale='1 1 «invocation.length»'>
				 <Shape> 
					...
					<Appearance>
						<Material
							diffuseColor='«RDSettings.METHOD_INVOCATION_COLOR»'
							transparency='0'/>
					</Appearance>
				</Shape>
			</Transform>
