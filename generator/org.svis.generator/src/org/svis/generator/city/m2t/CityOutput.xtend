...
class CityOutput implements IGenerator2 {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext ig) {
		switch(CitySettings::OUTPUT_FORMAT) {
			case AFrame: {
			...
			case X3D: {
				switch(CitySettings::BUILDING_TYPE) {
					case BuildingType::CITY_ORIGINAL:	fsa.generateFile("model.x3d", toX3DHead + city2x3d.toX3DBody(resource)  + toX3DTail) 
					case BuildingType::CITY_PANELS,
					case BuildingType::CITY_FLOOR,
					case BuildingType::CITY_BRICKS:		fsa.generateFile("model.x3d", toX3DHead + city2x3d.settingsInfo + city2x3d.toX3DBody(resource) + toX3DTail) 
					case BuildingType::CITY_DYNAMIC: {
						fsa.generateFile("model.html", toX3DOMHead + toAnimationFramework().toString + city2x3dom.toX3DOMBody(resource) + toX3DOMTail) 
